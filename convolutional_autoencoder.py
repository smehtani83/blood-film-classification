...
class ConvolutionalAutoencoder(object):
    ...
    def __init__(self):
        ...
        x = tf.placeholder(tf.float32, shape=[None, 128, 128, 3])  # [#batch, img_height, img_width, #channels]

        # encode
        conv1 = Convolution2D([20, 20, 3, 128], activation=tf.nn.relu, scope='conv_1')(x)
        pool1 = MaxPooling(kernel_shape=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME', scope='pool_1')(conv1)   #check
        conv2 = Convolution2D([20, 20, 128, 128], activation=tf.nn.relu, scope='conv_2')(pool1)
        pool2 = MaxPooling(kernel_shape=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME', scope='pool_2')(conv2)
        unfold = Unfold(scope='unfold')(pool2)
        encoded = FullyConnected(4000, activation=tf.nn.relu, scope='encode')(unfold)
        # decode
        decoded = FullyConnected(32*32*128, activation=tf.nn.relu, scope='decode')(encoded)   #initially  7*7*32
        fold = Fold([-1, 32, 32, 128], scope='fold')(decoded)   # [-1,7,7,3  2]
        unpool1 = UnPooling((2, 2), output_shape=tf.shape(conv2), scope='unpool_1')(fold)
        deconv1 = DeConvolution2D([20, 20, 128, 128], output_shape=tf.shape(pool1), activation=tf.nn.relu, scope='deconv_1')(unpool1)
        unpool2 = UnPooling((2, 2), output_shape=tf.shape(conv1), scope='unpool_2')(deconv1)
        reconstruction = DeConvolution2D([20, 20, 3, 128], output_shape=tf.shape(x), activation=tf.nn.sigmoid, scope='deconv_2')(unpool2)
...
def main():
    ...
    conv_autoencoder.train(batch_size=30, passes=5000, new_training=True)   #25,1000
